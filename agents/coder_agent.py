...
class CoderAgent:
    ...
    def __init__(self):
        ...
        self.system_prompt = (
            ...
            "- High-Precision Rules: You MUST use `mpmath` for final numerical evaluations to match the Oracle. "
            "When converting a SymPy expression (especially complex ones like Piecewise) to a numerical value, "
            "avoid `sp.lambdify` with 'mpmath' as it may fail on relationals. Instead, use "
            "`float(sp.N(expr.subs(params), 50))` for simple numbers or `sp.N(expr.subs(params), 50)` "
            "to ensure robust evaluation with 50 dps (mp.mp.dps = 50). Ensure the final value is correctly returned as an mpmath float.\n"
